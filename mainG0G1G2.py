...
def main(opt):
    ...
    G2 = generator(4, 3)
    G0 = generator(4, 3)
    D1 = discriminator(7, 1)
    D2 = discriminator(7, 1)
    D0 = discriminator(7, 1)
    ...
    G0 = init_net(G0, init_type='normal', init_gain=0.2, gpu_ids=opt.gpu_ids)
    ...
    D0 = init_net(D0, init_type='normal', init_gain=0.2, gpu_ids=opt.gpu_ids)
    ...
    # get optimizer
    optimizerD = torch.optim.Adam([{'params': D0.parameters()}, {'params': D1.parameters()}, {'params': D2.parameters()}],
                                  lr=opt.lr,
                                  betas=(opt.beta1, 0.999))
    optimizerG = torch.optim.Adam([{'params': G0.parameters()}, {'params': G1.parameters()}, {'params': G2.parameters()}],
                                  lr=opt.lr,
                                  betas=(opt.beta1, 0.999))
    ...
    for epoch in range(opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            fake_A = G0(torch.cat((real_B, real_C), 1))
            fake_B = G1(torch.cat((real_C, fake_A), 1))
            # fake_B = fake_B_pool.query(fake_B)
            fake_C = G2(torch.cat((fake_A, fake_B), 1))

            # update D1, D2
            set_requires_grad([D1, D2, D0], True)  # enable backprop for D1, D2
            # optimizerD1.zero_grad()  # set D1's gradients to zero
            # optimizerD2.zero_grad()  # set D2's gradients to zero
            optimizerD.zero_grad()

            """Calculate GAN loss for the discriminator"""
            # calculate gradients for D0
            fake_BCA = torch.cat((real_B, real_C, fake_A), 1)
            pred_fake = D0(fake_BCA.detach())
            ...
            loss_D0_fake = criterionGAN(pred_fake, label_d_fake)

            real_BCA = torch.cat((real_B, real_C, real_A), 1)
            pred_real = D0(real_BCA)
            loss_D0_real = criterionGAN(pred_real, label_d_real)
            loss_D0 = loss_D0_fake + loss_D0_real

            # calculate gradients for D1
            ## Fake; stop backprop to the generator by detaching fake_B
            # we use conditional GANs; we need to feed both input and output to the discriminator
            fake_CAB = torch.cat((real_C, fake_A, fake_B), 1)
            pred_fake = D1(fake_CAB.detach())
            loss_D1_fake = criterionGAN(pred_fake, label_d_fake)
            ## Real
            real_CAB = torch.cat((real_C, real_A, real_B), 1)
            pred_real = D1(real_CAB)
            ...
            fake_ABC = torch.cat((fake_A, fake_B, fake_C), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            ...
            loss_D = opt.lambda2 * loss_D1 + opt.lambda3 * loss_D2 + loss_D0 * 0.1
            loss_D.backward()
            ...
            set_requires_grad([D1, D2, D0], False)
            ...
            # calculate gradients for G0
            fake_BCA = torch.cat((real_B, real_C, fake_A), 1)
            pred_fake = D0(fake_BCA.detach())
            loss_G0_GAN = criterionGAN(pred_fake, label_d_real)
            loss_G0_L1 = criterionL1(fake_A, real_A)
            loss_G0 = loss_G0_GAN * 0.1 + loss_G0_L1
            ...
            loss_G = loss_G1 + loss_G2 + loss_G0
            loss_G.backward()
            ...
            if total_iters % opt.print_freq == 0:    # print training losses and save logging information to the disk
                losses = {'loss_G': loss_G.item(), 'loss_D': loss_D.item(),
                          ...
                          'loss_G0_GAN': loss_G0_GAN.item(), 'loss_G0_L1': loss_G0_L1.item(),
                          'loss_D0_real': loss_D0_real.item(), 'loss_D0_fake': loss_D0_fake.item(),
                          }
            ...
            if total_iters % opt.display_freq == 0:  # save images
                visuals = { 'fake_A': fake_A, 'real_A': real_A,
                ...
                for i in range(opt.batchSize):
                    for label, image in visuals.items():
                        ...
                        idx = idx + 1
                vutils.save_image(batch_vis, '%s/train_results/generated_epoch_%06d_iter%06d.png' % \
            ...
            if total_iters % opt.evalIter == 0:   # evaluation
                ...
                val_batch_output_A = Tensor(np.zeros(val_A.size()))
                # BER = []
                # RMSE = []
                for idx in range(val_A.size(0)):
                    ...
                    with torch.no_grad():
                        ...
                        val_input_C = Variable(val_C_single_img)
                    fake_A = G0(torch.cat([val_input_B, val_input_C], 1))
                    fake_B = G1(torch.cat([val_input_C, fake_A], 1))
                    fake_C = G2(torch.cat([fake_A, fake_B], 1))
                    ...
                    val_batch_output_A[idx, :, :, :].copy_(fake_A.data.squeeze(0))
                ...
                vutils.save_image(val_batch_output_C, '%s/val_results/generated_epoch_%06d_iter%06d_C.png' % \
                                  (opt.output_dir, epoch, total_iters), normalize=True)
                vutils.save_image(val_batch_output_A, '%s/val_results/generated_epoch_%06d_iter%06d_A.png' % \
                                  (opt.output_dir, epoch, total_iters), normalize=True)
        ...
        if epoch % opt.save_epoch_freq == 0:  # cache our model every <save_epoch_freq> epochs
            ...
            if len(opt.gpu_ids) > 0 and torch.cuda.is_available():
                ...
                torch.save(G0.module.cpu().state_dict(), '%s/G2_epoch_%d.pth' % (opt.output_dir, epoch))
                torch.save(D0.module.cpu().state_dict(), '%s/D2_epoch_%d.pth' % (opt.output_dir, epoch))
                G1.cuda(opt.gpu_ids[0])
                G2.cuda(opt.gpu_ids[0])
                G0.cuda(opt.gpu_ids[0])
                D1.cuda(opt.gpu_ids[0])
                D2.cuda(opt.gpu_ids[0])
                D0.cuda(opt.gpu_ids[0])
            else:
                ...
                torch.save(G0.cpu().state_dict(), '%s/G2_epoch_%d.pth' % (opt.output_dir, epoch))
                torch.save(D0.cpu().state_dict(), '%s/D2_epoch_%d.pth' % (opt.output_dir, epoch))
